...
class BancEssaiApp:
    def __init__(self, root):
        ...
        # Continuité stop/reprise
        self._last_course_mm = 0.0
        self._last_time_s = 0.0
        # Pile d'annulation (Ctrl+Z)
        self._undo_stack = []
        # Fixed params
    ...
    def start(self):
        ...
        # Continuité de position : si des données existent déjà, reprendre depuis
        # la dernière position connue (évite le retour à 0 à la reprise).
        initial_pos_m = self._last_course_mm / 1000.0 if not self.data_manager.is_empty else 0.0
        time_offset = self._last_time_s if not self.data_manager.is_empty else 0.0
    ...
    def stop(self):
        ...
        # Sauvegarder la dernière position et le temps pour la reprise
        if was_acquiring and not self.data_manager.is_empty:
            latest = self.data_manager.get_latest_values()
            if latest:
                self._last_time_s = latest[0]
                self._last_course_mm = latest[2]
    ...
    def _check_offset_after_acquisition(self):
        ...
        if answer:
            ...
            self._undo_push('apply_offset', effort_offset=apply_eff, course_offset=apply_crs)
            self.update_plot()
    ...
    # =================== UNDO (Ctrl+Z) ===================
    def _undo_push(self, action_type, **kwargs):
        """Enregistre une action annulable dans la pile."""
        self._undo_stack.append({'type': action_type, **kwargs})
    ...
    def undo(self):
        ...
        atype = action['type']
        if atype == 'add_point':
            # Annuler l'ajout d'un point : le supprimer
            idx = action.get('index')
            if idx is not None:
                self.data_manager.remove_point(idx)
                self.update_plot()
        elif atype == 'remove_point':
            # Annuler la suppression d'un point : le restaurer
            p = action['point_data']
            self.data_manager.add_point(
                p['x'], p['y'], point_type=p.get('type', 'course'),
                curve_name=p.get('curve'), color=p.get('color'),
                name=p.get('name'), mode=p.get('mode'))
            self.update_plot()
        elif atype == 'add_curve':
            # Annuler l'ajout d'une courbe : la supprimer
            self.data_manager.remove_curve(action['curve_name'])
            self.update_plot()
        elif atype == 'remove_curve':
            # Annuler la suppression d'une courbe : la restaurer
            self.data_manager.add_curve(
                action['curve_name'], action['curve_data'], action['color'])
            self.update_plot()
        elif atype == 'add_smooth':
            # Annuler le lissage : supprimer la courbe lissée
            self.data_manager.remove_curve(action['smooth_name'])
            self.update_plot()
        elif atype == 'apply_offset':
            # Annuler l'offset : appliquer l'offset inverse
            self.data_manager.apply_offset(
                -action['effort_offset'], -action['course_offset'])
            self.update_plot()
        elif atype == 'clear_main':
            # Annuler l'effacement : restaurer les données principales
            self.data_manager.set_data_from_file(
                action['time'], action['effort'], action['course'],
                action['raw_effort'], action['raw_course'])
            self.update_plot()
    ...
    def update_plot(self):
        ...
        if mode == "sensor":
            ...
            if data['course'] and data['effort']:
                # Tracé en ordre temporel (pas trié par course) pour afficher
                # correctement l'hystérésis aller/retour sans "remplissage".
                self.plot_manager.plot_main_curve(data['course'], data['effort'], label=self.t("legend_essai"), color=self.curve_color)
    ...
    def refresh_for_new_measurement(self):
        ...
        self._last_course_mm = 0.0; self._last_time_s = 0.0
    ...
    def add_curve(self):
        ...
        try:
            ...
            self.data_manager.add_curve(name, cd, color)
            self._undo_push('add_curve', curve_name=name)
            self.update_plot()
        except Exception as e: messagebox.showerror("Erreur", str(e))
    ...
    def _add_point_interactive(self, point_type="course"):
        ...
        def val():
            ...
            if len(matches) == 1:
                ...
                self._undo_push('add_point', index=len(self.data_manager.get_points()) - 1)
    ...
    def show_smooth_dialog(self):
        ...
        def apply():
            ...
            if result:
                ...
                self._undo_push('add_smooth', smooth_name=smooth_name)
                self.update_plot()
    ...
    def manage_graph_elements(self):
        ...
        def clear_main():
            if messagebox.askyesno(self.t("msg_confirm_delete"), self.t("msg_clear_main_confirm")):
                snap = self.data_manager.get_data_copy()
                if snap['time']:
                    self._undo_push('clear_main', time=snap['time'], effort=snap['effort'],
                                    course=snap['course'], raw_effort=snap['raw_effort'],
                                    raw_course=snap['raw_course'])
                self.data_manager.clear_main_data(); self.start_time = 0.0
                self.timer_label.config(text=f"⏱ {self.t('timer_prefix')} 0.0 s")
                self.update_plot(); win.destroy()
        ...
        def dc():
            ...
            if not s: return
            name = clb.get(s[0])
            if messagebox.askyesno(self.t("msg_confirm_delete"), f"Supprimer '{name}' ?"):
                snap = self.data_manager.get_data_copy()
                crv = snap['additional_curves'].get(name, {})
                self._undo_push('remove_curve', curve_name=name,
                                curve_data={k: v for k, v in crv.items() if k != 'color'},
                                color=crv.get('color', THEME["primary"]))
                self.data_manager.remove_curve(name); self.update_plot(); win.destroy()
        ...
        def dp():
            s = plb.curselection()
            if s:
                pts = self.data_manager.get_points()
                if s[0] < len(pts):
                    self._undo_push('remove_point', point_data=pts[s[0]])
                self.data_manager.remove_point(s[0]); self.update_plot(); win.destroy()
        def cap():
            if messagebox.askyesno(self.t("msg_confirm_delete"), self.t("btn_delete_all") + " ?"):
                self.data_manager.clear_points(); self.update_plot(); win.destroy()
    ...
    def show_tutorials(self):
        ...
        def _tut_mw(e): canvas.yview_scroll(int(-1 * (e.delta / 120)), "units"); return "break"
        canvas.bind("<MouseWheel>", _tut_mw)
        win.bind("<MouseWheel>", _tut_mw)
